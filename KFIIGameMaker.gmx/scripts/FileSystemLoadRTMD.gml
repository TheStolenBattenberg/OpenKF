///FileSystemLoadRTMD(fileID, overrideDir, collidersOut);
...
}else
{
    ...
    if(file_exists(global.FS[0]+arrFileRef[1]+".msm"))
    {
        ...
        MDL = MSMLoadFromFile(global.FS[0]+arrFileRef[1]+".msm", argument2);
        
    }else{
        //Load from T
        show_debug_message("Loading " + argument0 + " from T...");
        
        var temp = RTMDLoadFromBuffer(arrFileRef[0], arrFileRef[2]);  
        MSMModelSaveFromMeshBuilders(temp, global.FS[0]+arrFileRef[1]+".msm");      
        
        MDL = MSMModelBuildFromMeshBuilders(temp, argument2);                
        temp = -1;
    }
}
