...
for(var i = 0; i < numAnim; ++i)
{
    ...
    //First we have to see if the frame count is equal to 1... If it is, we need to add the base frame as a target too.
    var offJ = 0;
    if(arrAnimation[0] == 1 || arrAnimation[0] == 2)
    {
    ...
    for(var j = 0; j < arrAnimation[0]; ++j)
    {
        ...
        arrTargetList[offJ+j] = MODecompressMorphTarget(moBuffer, arrTargetOffset[arrFrame[1]], arrTarget);
    }    
        
    //Store changed information into animation structure
    arrAnimation[0] = arrAnimation[0] + offJ;
